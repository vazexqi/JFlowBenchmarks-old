...
/**
 * TODO add javadoc The E_list an Exp structures defined below comprise the
 * expression trees that are stored in the dictionary. The expression has a type
 * (AND, OR or TERMINAL). If it is not a terminal it has a list (an EList) of
 * children.
 */
public class Exp {
    ...
    int type;
    /*
      * The cost of using this expression. Only used for non-terminals
      */
    int cost;
    ...
    int mark_dead_connectors(Sentence sent, char d) {
        /*
           * Mark as dead all of the dir-pointing connectors in e that are not
           * matched by anything in the current set. Returns the number of
           * connectors so marked.
           */
        Connector dummy = new Connector();
        int count;
        ExpList el;
