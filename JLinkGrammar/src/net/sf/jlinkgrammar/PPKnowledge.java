...
/**
 * I also, unfortunately, want to propose a new type of domain. These would
 * include everything that can be reached from the root word of the link, to the
 * right, that is closer than the right word of the link. (They would not
 * include the link itself.)
 * <p/>
 * <p/>
 * In the following sentence, then, the "Urfl_Only Domain" of the G link would
 * include only the "O" link:
 * <p/>
 * <pre>
 *       +-----G----+
 *       +---O--+   +-AI+
 ...
 * </PRE>
 *
 * In the following sentence it would include the "O", the "TT", the "I", the
 * second "O", and the "A".
 *
 * <PRE>
 *       +----------------G---------------+
 ...
 *
 *
 * (These are distinctions that I thought we would never be able to make, so I
 * told myself they were semantic rather than syntactic. But with domains, they
 * should be easy.)
 *
 * Modifications
 *
 * 3) when postprocessing a sentence, the links of each domain are placed in a
 * set for quick lookup, ('contains one' and 'contains none')
 *
 * 7) observation: the 'contains one' is, empirically, by far the most violated
 * rule, so it should come first in applying the rules.
 */
public class PPKnowledge {
     ...
     * auxiliary data, necessary to implement the rules, below. See comments in
     * post-process.c for a description of these.
     */
    ...
    public PPKnowledge(ParseOptions opts, String dictname, String path)
            throws IOException {
        /*  */
     ...
     * connectivity is there, or it isn't. The only information in the rule
     * (besides its presence) is the error message to display if the rule is
     * violated
     */
    private void read_connected_rule(ParseOptions opts, String label) {
        ...
        if (lt.pp_lexer_count_tokens_of_label() > 1)
            throw new RuntimeException("postProcess: Invalid syntax in "
                    + label);
        ...
        for (r = 0; r < n_form_a_cycle_rules; r++) {
            ...
            if (n_tokens > 1)
                throw new RuntimeException(
                        "postProcess: Invalid syntax: rule " + (r + 1)
                                + " of " + label);
    ...
    /**
     * Reading the 'contains_one_rules' and reading the 'contains_none_rules'
    ...
    private PPRule[] read_contains_rules(ParseOptions opts, String label) {
        ...
        if (!lt.pp_lexer_set_label(label)) {
            nRules = 0;
            if (opts.verbosity > 0)
                opts.out.println("PP warning: Not using any " + label
                        + " rules");
        } else {
            n_commas = lt.pp_lexer_count_commas_of_label();
            nRules = (n_commas + 1) / 3;
    ...
    private void initialize_set_of_links_starting_bounded_domain() {
        ...
        set_of_links_starting_bounded_domain = PPLinkset
                .PPLinkset_open(GlobalBean.PP_MAX_UNIQUE_LINK_NAMES);
     ...
     * whose handle is returned. Return null if link set not defined in file, in
     * which case the set is taken to be empty.
     */
    private PPLinkset read_link_set(ParseOptions opts, String label) {
        ...
        if (!lt.pp_lexer_set_label(label)) {
            if (opts.verbosity > 0)
                opts.out.println("PP warning: Link set " + label
                        + " not defined: assuming empty.");
            n_strings = 0;
        } else
    ...
    private void read_link_sets(ParseOptions opts) {
        ...
        urfl_domain_starter_links = read_link_set(opts,
        ...
        urfl_only_domain_starter_links = read_link_set(opts,
                "URFL_ONLY_DOMAIN_STARTER_LINKS");
        left_domain_starter_links = read_link_set(opts,
                "LEFT_DOMAIN_STARTER_LINKS");
    }

    /**
     * read table of [link, domain type]. This tells us what domain type each
     * link belongs to. This lookup table *must* be defined in the knowledge
     * file.
     */
    private void read_starting_link_table() {
        ...
        if (!lt.pp_lexer_set_label(label))
            throw new RuntimeException(
                    "postProcess: Couldn't find starting link table " + label);
        n_tokens = lt.pp_lexer_count_tokens_of_label();
        if ((n_tokens % 2) != 0)
            throw new RuntimeException(
                    "postProcess: Link table must have format [<link> <domain name>]+");
        ...
        for (i = 0; i < nStartingLinks; i++) {
            ...
            starting_link_lookup_table[i].starting_link = lt
    ...
    static void check_domain_is_legal(String p) {
        if (p.length() > 1)
            throw new RuntimeException("postProcess: Domain (" + p
