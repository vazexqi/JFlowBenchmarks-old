...
/**
 * The linkage routines are primarily used to walk linkages and retrieve
 * information about the sentence. You must fist call dictionary_create,
 * sentence,_create, sentenceParse, in that order. Then you can process links.
 *
...
public class Linkage {
    ...
    /*
      ...
      * One potential problem is changing parse options in one linkage does not
      * propogate to other linkages as well. Call it a design decision but you
      * should be aware that post processing one linkage with a differrent set of
      * options will not propogate those parse options to other linkages. For
      * those of you looking for the linkage_create function from the C API this
      * is it. Use Linkage myLinkage = new Linkage(int, Sentence, ParseOptions);
      * where you formerly used: Linkage l; l = linkage_create(int, Sentence,
      * ParseOptions);
      *
      * Caveat - The creator function calls post process. If you want a
      * non-standard post-processor you will have to call
      * linkage_post_porcessor(PostProcessor p) yourself.
      *
      * Each sentence has an array of linkages. To find out how many call num =
      * mySentence.numLinkagesFound() on your instantiated sentence.
      * You can then loop from 0 to num and then call
      *
      * @see linkage_get_num_sublinkages()
      *
      * @see linkage_set_current_subinkage(int index) linkage_compute_union()
      * linkage_get_num_words() linkage_get_num_links()
      * linkage_get_link_length(int index) - number of words spanned by link
      * linkage_get_link_lword(int link) linkage_get_link_rword(int link)
      * linkage_get_link_label(int link) linkage_get_link_llabel(int link)
      * linkage_get_link_rlabel(int link) linkage_get_word(int wordFromLink)
      *
      * TODO - There is some sloppy coding (or thinking in this creator) the sent
      * argument overides this.sent but this.sent is coppied at the beginning of
      * the method and not at the end. Therefore the object's sentence
      * (this.sent) may be out of sync with the sentence it analyzes. Also new
      * linakges can alter the enclosing sentence object. Some better object work
      ...
      *
      * @param opts set the linkage objects private copy of the ParseOptions to
      * opts
      *
      * @return if the index is valid it creates a new linkage if the link
      * already exists it throws a runtime exception "index out of range"
      */
    ...
    void compute_chosen_words(Sentence sent) {
        /*
           * This takes the current chosen_disjuncts array and uses it to compute
           * the chosen_words array. "I.xx" suffixes are eliminated.
           */
        int i, l;
        String s, t, u;
     ...
     * For this to work, you must have already called parse, and already built
     * the whole_set.
     */
     ...
     *
     * @param sent sentence to extract links from. TODO - does not set the
     *             current Linkage objects this.sent to sent. This may be a bug!
     * @param opts options to use when processing TODO - does not set the current
     *             Linkage objects this.opts to opts. This may be a bug!
     */
    ...
    /**
     * The old C comment said -- This procedure mimics analyzeFatLinkage in
     * order to extract the sublinkages and copy them to the Linkage data
     * structure passed in. BUT no linkage structure is passed in the Linkage
     * structure is the instance variable of this Linkage.
     *
     * @param sent sentence to extract links from. TODO - does not set the
     *             current Linkage objects this.sent to sent. This may be a bug!
     * @param opts options to use when processing TODO - does not set the current
    ...
    public void extract_fat_linkage(Sentence sent, ParseOptions opts) {
        ...
        for (; ; ) {
            ...
            sent.fillPatchArrayDis(d_root, null);
            ...
            sent.computePPLinkArrayConnectors(sublinkage);
            sent.computePPLinkNames(sublinkage);
    ...
    /**
     * This is a toplevel routine used to setup a for loop to process each
     * sublinkage (usually an AND disjunct).
     *
    ...
    /**
     * This is need to set up the for loop to walk down a set of links for
     * printing or other processing returns the size of the linkage array as the
     * number of links
     *
    ...
    /**
     * A method used to find the current number of linkages. To create a new
     * linkage you will have to add one to the result of this method It uses a
     * looop to find the next available linkage index
     *
    ...
    /**
     * setter function to set the current sublinkage to indicated index TODO -
     * does this function have to insure a compact array? If so should it return
        ...
        for (i = 0; i < num_sublinkages; ++i) {
            for (j = 0; j < sublinkage[i].num_links; ++j) {
                ...
                if (!link_already_appears(link, i)) {
                    ...
                    if (((p = sublinkage[i].violation) != null)
                            && (u.violation == null)) {
                        u.violation = p;
                    }
                    num_in_union++;
     ...
     *
     * @param s    Empty stringbuffer supplied by the caller shich the method
     *             will use
    ...
    public void print_a_link(StringBuffer s, int link) {
        ...
        if ((l == 0) && dict.left_wall_defined) {
            GlobalBean.leftAppendString(s, GlobalBean.LEFT_WALL_DISPLAY,
                    "               ");
        } else if ((l == (linkage_get_num_words() - 1))
                && dict.right_wall_defined) {
            GlobalBean.leftAppendString(s, GlobalBean.RIGHT_WALL_DISPLAY,
                    "               ");
        } else {
            GlobalBean.leftAppendString(s, linkage_get_word(l),
                    "               ");
        }
        GlobalBean.leftAppendString(s, llabel, "     ");
        s.append("   <---");
        GlobalBean.leftAppendString(s, label, "-----");
        s.append("->  ");
        GlobalBean.leftAppendString(s, rlabel, "     ");
        s.append("     ");
            ...
            if (!opts.display_walls)
                for (j = 0; j < N_links; j++) {
                    if (ppla[j].l == 0) {
                        ...
                        N_wall_connectors++;
                        if (ppla[j].lc.string
                                .equals(GlobalBean.LEFT_WALL_SUPPRESS)) {
        ...
        if (dict.right_wall_defined) {
            ...
            for (j = 0; j < N_links; j++) {
                if (ppla[j].r == num_words - 1) {
                    N_wall_connectors++;
                    if (ppla[j].lc.string
                            .equals(GlobalBean.RIGHT_WALL_SUPPRESS)) {
                        suppressor_used = true;
                    }
                }
            }
    ...
    public String linkage_print_diagram() {
        ...
        String gr_string;
        Dictionary dict = sent.dict;
        int x_screen_width = opts.getScreenWidth();
        ...
        for (link_length = 1; link_length < N_words_to_print; link_length++) {
            for (j = 0; j < N_links; j++) {
                ...
                if (opts.display_link_subscripts)
                    ...
                    while (k < connector.length()
                            && Character.isUpperCase(connector.charAt(k)))
     ...
     *
     * @param mode mode 1: treebank-style constituent tree mode 2: flat,
     *             bracketed tree [A like [B this B] A] mode 3: flat,
     *             treebank-style tree (A like (B this) )
     * @return Returns the constituent tree in one of three modes, treebank,
     *         flat bracketed, or flat treebank
     */
    ...
    public String linkage_print_constituent_tree(int mode) {
        ...
        throw new RuntimeException(
                "Illegal mode in linkage_print_constituent_tree");
    }
    ...
    int token_type(String token) {
        ...
        if (token.length() > 1
                && token.charAt(token.length() - 1) == GlobalBean.CLOSE_BRACKET)
    ...
    public CNode parse_string(CNode n, StringTokenizer tok) {
        ...
        while (tok.hasMoreTokens()) {
            ...
            switch (token_type(q)) {
                ...
                default:
                    throw new RuntimeException(
                            "Constituent tree: Illegal token type");
            }
            if (n.child == null) {
        ...
        throw new RuntimeException(
                "Constituent tree: Constituent did not close");
    }
     ...
     * String linkage_print_constituent_tree(int mode);
     *
     * @return A CNode is a standard tree data structure. The children of a node
     *         are stored as a linked list, with the end of the list indicated
     *         by next==NULL. The start and end fields of a node indicate the
     *         span of the constituent, with the first word indexed by 0. Leaves
     *         are defined by the condition child==NULL.
     */
     ...
     *             bracketed tree [A like [B this B] A] mode 3: flat,
     *             treebank-style tree (A like (B this) )
     * @return A Java String version of a postscript file that can be sent to a
     *         print device.
     */
    ...
    private void count_words_used() {
        ...
        /*
           * This function generates a table, word_used[i][w], showing whether
           * each word w is used in each sublinkage i; if so, the value for that
           * cell of the table is 1
           */
    ...
    private String exprint_constituent_structure(int numcon_total) {
        ...
        for (w = 1; w < num_words; w++) {
            /*
                * Skip left wall; don't skip right wall, since it may have
            ...
            while (true) {
                ...
                bestright = -1;
                for (c = 0; c < numcon_total; c++) {
                    if ((constituent[c].left == w) && (leftdone[c] == 0)
            ...
            if (w < num_words - 1) {
                ...
                /*
                     * Now, if the first character of the word was originally
                     * uppercase, we put it back that way
                     */
            ...
            while (true) {
                ...
                bestleft = -1;
                for (c = 0; c < numcon_total; c++) {
                    if ((constituent[c].right == w) && (rightdone[c] == 0)
                            && (constituent[c].valid == 1)
    ...
    private int gen_comp(int numcon_total, int numcon_subl, String ctype1,
                         String ctype2, String ctype3, int x) {

        /*
           * This function looks for constituents of type ctype1. Say it finds
           * one, call it c1. It searches for the next larger constituent of type
           * ctype2, call it c2. It then generates a new constituent of ctype3,
           * containing all the words in c2 but not c1.
           */
        ...
        for (c1 = numcon_total; c1 < numcon_total + numcon_subl; c1++) {
            ...
            if ((x == 4)
                    && !Postprocessor.post_process_match("MX#*",
                    constituent[c1].start_link))
                continue;

            /*
                * If ctype1 is X, and domain_type is t, it's an infinitive - skip
                * it
                */
            ...
            /*
                * If it's domain-type z, it's a subject-relative Clause; the VP
                * doesn't need an NP
                */
            ...
            /*
                * If ctype1 is X or VP, and it's not started by an S, don't
                * generate an NP (Neither of the two previous checks are necessary
                * now, right?)
                */
            if ((x == 1 || x == 2)
                    && ((!Postprocessor.post_process_match("S",
                    constituent[c1].start_link)
                    && !Postprocessor.post_process_match("SX",
                    constituent[c1].start_link) && !Postprocessor
                    .post_process_match("SF",
                            constituent[c1].start_link)) || Postprocessor
                    .post_process_match("S##w",
                            constituent[c1].start_link)))
                continue;

            /*
                * If it's an SBAR (relative Clause case), it has to be a relative
                * Clause
                */
            if ((x == 3)
                    && (!Postprocessor.post_process_match("Rn",
                    constituent[c1].start_link)
                    && !Postprocessor.post_process_match("R*",
                    constituent[c1].start_link)
                    && !Postprocessor.post_process_match("MX#r",
                    constituent[c1].start_link)
                    && !Postprocessor.post_process_match("Mr",
                    constituent[c1].start_link) && !Postprocessor
                    .post_process_match("MX#d",
                            constituent[c1].start_link)))
                continue;
            ...
            if ((x == 7)
                    && !Postprocessor.post_process_match("SI",
                    constituent[c1].start_link))
                continue;

            /*
                * If ctype1 is VP (participle modifier case), it has to be started
                * by an Mv or Mg
                */
            if ((x == 8)
                    && !Postprocessor.post_process_match("M",
                    constituent[c1].start_link))
                continue;

            /*
                * If ctype1 is VP (participle opener case), it has to be started by
                * a COp
                */
            if ((x == 9)
                    && !Postprocessor.post_process_match("COp",
                    constituent[c1].start_link))
                continue;

            /*
                * Now start at the bounds of c1, and work outwards until you find a
                * larger constituent of type ctype2
                */
            if (!(constituent[c1].type.equals(ctype1)))
                continue;

            if (opts.verbosity >= 2)
                opts.out.print("Generating complement constituent for c " + c1
                        + " of type " + ctype1);
            done = 0;
            for (w2 = constituent[c1].left; (done == 0) && (w2 >= 0); w2--) {
                for (w3 = constituent[c1].right; w3 < num_words; w3++) {
                    for (c2 = numcon_total; (done == 0)
                        ...
                        if (!((constituent[c2].left == w2) && (constituent[c2].right == w3))
                                || (c2 == c1))
                            continue;
                        ...
                        /*
                               * if the new constituent (c) is to the left of c1, its
                               * right edge should be adjacent to the left edge of c1
                               * - or as close as possible without going outside the
                               * current sublinkage. (Or substituting right and left
                               * as necessary.)
                               */

                        if ((x == 5) || (x == 6) || (x == 9)) {
                            /*
                                    * This is the case where c is to the RIGHT of c1
                                    */
                        ...
                        if (opts.verbosity >= 2) {
                            opts.out.print("Larger c found: c " + c2 + " ("
                                    + ctype2 + "); ");
            ...
            if (opts.verbosity >= 2) {
                if (done == 0)
                    opts.out.println("No constituent added, because no larger "
                            + ctype2 + " was found");
            }
        }
    ...
    public void adjust_subordinate_clauses(int numcon_total, int numcon_subl) {
        ...
        /*
           * Look for a constituent started by an MVs or MVg. Find any VP's or
           * ADJP's that contain it (without going beyond a larger S or NP).
           * Adjust them so that they end right before the m domain starts.
           */
        ...
        for (c = numcon_total; c < numcon_total + numcon_subl; c++) {
            if (Postprocessor.post_process_match("MVs",
                    ...
                    || Postprocessor.post_process_match("MVg",
                    constituent[c].start_link)) {
                done = 0;
                for (w2 = constituent[c].left - 1; (done == 0) && w2 >= 0; w2--) {
                    for (c2 = numcon_total; c2 < numcon_total + numcon_subl; c2++) {
                        if (!((constituent[c2].left == w2) && (constituent[c2].right >= constituent[c].right)))
                            continue;
                        if ((constituent[c2].type.equals("S"))
                                || (constituent[c2].type.equals("NP"))) {
                            done = 1;
                            break;
                        }
    ...
    private void print_constituent(int c) {
        int w;
        /*
           * Sentence sent; sent = linkage_get_sentence(linkage); **PV* using
           * linkage.word not sent.word
           */
        if (opts.verbosity < 2)
            return;
        opts.out.print("  c " + c + " " + constituent[c].type + " ["
                + constituent[c].domain_type + "] (" + constituent[c].left
                + "-" + constituent[c].right + "): ");
        for (w = constituent[c].left; w <= constituent[c].right; w++) {
            opts.out.print(word[w]);
            /** PV **/
            opts.out.print(" ");
        }
        opts.out.println();
    ...
    private void adjust_for_left_comma(int c) {
        ...
        /*
           * If a constituent c has a comma at either end, we exclude the comma.
           * (We continue to shift the boundary until we get to something inside
    ...
    private void adjust_for_right_comma(int c) {
        ...
        if ((word[constituent[c].right].equals(","))
                || (word[constituent[c].right].equals("RIGHT-WALL"))) {
            w--;
            while (true) {
                if (word_used[current][w] == 1)
                    break;
    ...
    private void print_tree(StringBuffer cs, boolean indent, CNode n, int o1,
                            int o2) {
        int i, child_offset;
        CNode m;
    ...
    private int read_constituents_from_domains(int numcon_total, int s) {
        ...
        for (d = 0, c = numcon_total; d < subl.pp_data.N_domains; d++, c++) {
            ...
            if ((domain.type == 'c') || (domain.type == 'd')
                    || (domain.type == 'e') || (domain.type == 'f')
                    || (domain.type == 'g') || (domain.type == 'u')
                    || (domain.type == 'y')) {
                leftlimit = 0;
            ...
            /*
                * Start by assigning both left and right limits to the right word
                * of the start link. This will always be contained in the
                * constituent. This will also handle the case where the domain
                * contains no links.
                */
            ...
            for (dlink = domain.lol; dlink != null; dlink = dlink.next) {
                ...
                if ((linkage_get_link_lword(l) < leftmost)
                        && (linkage_get_link_lword(l) >= leftlimit))
            ...
            if (constituent[c].domain_type == '\0') {
                throw new RuntimeException(
                        "Error: no domain type assigned to constituent\n");
            }
            if (constituent[c].start_link == null) {
                throw new RuntimeException(
                        "Error: no type assigned to constituent\n");
            }
        }
        ...
        if (opts.verbosity >= 2)
            opts.out.println("Constituents added at first stage for subl "
                    + current + ":");
        for (c = numcon_total; c < numcon_total + numcon_subl; c++) {
            print_constituent(c);
        }
        ...
        /*
           * Opener case - generates S around main Clause. (This must be done
           * first; the S generated will be needed for later cases.)
           */
        ...
        /*
           * Make sure the constituents are nested. If two constituents are not
           * nested: whichever constituent has the furthest left boundary, shift
           * that boundary rightwards to the left boundary of the other one
           */
        ...
        /*
           * This labels certain words as auxiliaries (such as forms of "be" with
           * passives, forms of "have" wth past participles, "to" with
           * infinitives). These words start VP's which include them. In Treebank
           * I, these don't get printed unless they're part of an andlist, in
           * which case they get labeled "X". (this is why we need to label them
           * as "aux".) In Treebank II, however, they seem to be treated just like
           * other verbs, so the "aux" stuff isn't needed.
           */

        for (c = numcon_total; c < numcon_total + numcon_subl; c++) {
            constituent[c].subl = current;
            if (((constituent[c].domain_type == 'v') && (wordtype[linkage_get_link_rword(constituent[c].start_num)] == GlobalBean.WType_PTYPE))
                    || ((constituent[c].domain_type == 't') && (constituent[c].type
                    .equals("VP")))) {
                constituent[c].aux = 1;
            } else
                constituent[c].aux = 0;
    ...
    private int find_next_element(int start, int numcon_total,
                                  int num_elements, int num_lists) {
        /*
           * Here we're looking for the next andlist element to add on to a
           * conjectural andlist, stored in the array templist. We go through the
           * constituents, starting at "start".
           */
        ...
        for (c = start + 1; c < numcon_total; c++) {
            ...
            /*
                * We're considering adding constituent c to the andlist. If c is in
                * the same sublinkage as one of the other andlist elements, don't
                * add it. If it overlaps with one of the other constituents, don't
                * add it. If there's a constituent identical to c that occurs in a
                * sublinkage in which one of the other elements occurs, don't add
                * it.
                */
            ...
            for (a = 0; a < num_elements; a++) {
                if (constituent[c].subl == constituent[templist[a]].subl)
                    ok = 0;
                if (((constituent[c].left < constituent[templist[a]].left) && (constituent[c].right > constituent[templist[a]].left))
                        || ((constituent[c].right > constituent[templist[a]].right) && (constituent[c].left < constituent[templist[a]].right))
                        || ((constituent[c].right > constituent[templist[a]].right) && (constituent[c].left < constituent[templist[a]].right))
                        || ((constituent[c].left > constituent[templist[a]].left) && (constituent[c].right < constituent[templist[a]].right)))
                    ok = 0;
            ...
            addedone = 1;
            num_lists = find_next_element(c, numcon_total, num_elements + 1,
                    num_lists);
        }
    ...
    private void generate_misc_word_info() {
        ...
        /*
           * Go through all the words. If a word is on the right end of an S (or
           * SF or SX), wordtype[w]=STYPE. If it's also on the left end of a Pg*b,
           * I, PP, or Pv, wordtype[w]=PTYPE. If it's a question-word used in an
           * indirect question, wordtype[w]=QTYPE. If it's a question-word
           * determiner, wordtype[w]=QDTYPE. Else wordtype[w]=NONE. (This function
           * is called once for each sublinkage.)
           */
        ...
        for (l1 = 0; l1 < linkage_get_num_links(); l1++) {
            ...
            if (uppercompare(label1, "S") || uppercompare(label1, "SX")
                ...
                for (l2 = 0; l2 < linkage_get_num_links(); l2++) {
                    ...
                    if ((w1 == w2)
                            && (Postprocessor
                            .post_process_match("Pg#b", label2)
                            || uppercompare(label2, "I")
                            || uppercompare(label2, "PP") || Postprocessor
                            .post_process_match("Pv", label2))) {
                        /* Pvf, Pgf? */
    ...
    private int last_minute_fixes(int numcon_total) {
        ...
        for (c = 0; c < numcon_total; c++) {
            ...
            /*
                * In a paraphrase construction ("John ran, he said"), the
                * paraphrasing Clause doesn't get an S. (This is true in Treebank
                * II, not Treebank I)
                */
            ...
            /*
                * If it's a possessive with an "'s", the NP on the left should be
                * extended to include the "'s".
                */
            ...
            /*
                * If a constituent has starting link MVpn, it's a time expression
                * like "last week"; label it as a noun phrase (incorrectly)
                */
            ...
            /*
                * If the constituent is an S started by "but" or "and" at the
                * beginning of the sentence, it should be ignored.
                */

            if ((constituent[c].start_link.equals("Wdc"))
                    && (constituent[c].left == 2)) {
                constituent[c].valid = 0;
            }

            /*
                * For prenominal adjectives, an ADJP constituent is assigned if
                * it's a hyphenated (Ah) or comparative (Am) adjective; otherwise
                * no ADJP is assigned, unless the phrase is more than one word long
                * (e.g. "very big"). The same with certain types of adverbs.
                */
            /*
                * That was for Treebank I. For Treebank II, the rule only seems to
                * apply to prenominal adjectives (of all kinds). However, it also
                * applies to number expressions ("QP").
                */

            if (Postprocessor
                    .post_process_match("A", constituent[c].start_link)
            ...
            if ((constituent[c].domain_type == 'h')
                    && (word[constituent[c].left - 1].equals("$"))) {
                constituent[c].left--;
            }

            /*
                * If a constituent has type VP and its aux value is 2, this means
                * it's an aux that should be printed; change its type to "X". If
                * its aux value is 1, set "valid" to 0. (This applies to Treebank I
                * only)
                */
        ...
        /*
           * If there's a global S constituent that includes everything except a
           * final period or question mark, extend it by one word
           */
        ...
        /*
           * If there's no S boundary at the very left end of the sentence, or the
           * very right end, create a new S spanning the entire sentence
           */
        ...
        for (c = 0; c < numcon_total; c++) {
            if ((constituent[c].left == 1) && (constituent[c].type.equals("S"))
                    && (constituent[c].valid == 1))
        ...
        /*
           * Check once more to see if constituents are nested (checking BETWEEN
           * sublinkages this time)
           */
    ...
    private int merge_constituents(int numcon_total) {
        ...
        for (c1 = 0; c1 < numcon_total; c1++) {
            ...
            if (constituent[c1].right < constituent[c1].left) {
                if (opts.verbosity >= 2)
                    opts.out.println("WARNING: Constituent " + c1
                            + " has negative length. Deleting it.");
        ...
        /*
           * First go through and give each constituent a canonical number (the
           * index number of the lowest-numbered constituent identical to it)
           */
        ...
        /*
           * If constituents A and B in different sublinkages X and Y have one
           * endpoint in common, but A is larger at the other end, and B has no
           * duplicate in X, then declare B invalid. (Example:
        ...
        for (c1 = 0; c1 < numcon_total; c1++) {
            ...
            for (c2 = 0; c2 < numcon_total; c2++) {
                ...
                ok = 1;
                /*
                     * Does c2 have a duplicate in the sublinkage containing c1? If
                     * so, bag it
                     */
                for (c3 = 0; c3 < numcon_total; c3++) {
        ...
        /*
           * Now go through and find duplicates; if a pair is found, mark one as
           * invalid. (It doesn't matter if they're in the same sublinkage or not)
           */
        ...
        /*
           ...
           * exclusive constituents. Each constituent in the list may not be
           * present in the same sublinkage as any of the others.
           */
        ...
        for (c1 = 0; c1 < numcon_total; c1++) {
            ...
            num_elements = 1;
            templist[0] = c1;
            num_lists = find_next_element(c1, numcon_total, num_elements,
                    num_lists);
        }
        ...
        /*
           ...
           * andlists--e.g. if andlist X contains constituents A and B, and Y
           * contains A B and C, we throw out X
           */
        ...
        /*
           * If an element of an andlist contains an element of another andlist,
           * it must contain the entire andlist.
           */
            ...
            for (a = 0; (a < andlist[n].num) && (andlist[n].valid); a++) {
                for (n2 = 0; (n2 < num_lists) && (andlist[n].valid); n2++) {
                    ...
                    for (a2 = 0; (a2 < andlist[n2].num) && (andlist[n].valid); a2++) {
                        ...
                        if (c1 == c2)
                            continue;
                        if (!((constituent[c2].left <= constituent[c1].left) && (constituent[c2].right >= constituent[c1].right)))
                            continue;
                        if (opts.verbosity >= 2)
                            opts.out.println("Found that c" + c2 + " in list "
                                    + n2 + "  is bigger than c" + c1
                                    + " in list " + n);
                        ok = 1;

                        /*
                               * An element of n2 contains an element of n. Now, we
                               * check to see if that element of n2 contains ALL the
                               * elements of n. If not, n is invalid.
                               */
                        ...
                        andlist[n].valid = false;
                        if (opts.verbosity >= 2) {
                            opts.out.print("Eliminating andlist, n=" + n
                                    + ", a=" + a + ", n2=" + n2 + ", a2=" + a2
                                    + ": ");
                            for (a3 = 0; a3 < andlist[n].num; a3++) {
        ...
        for (n = 0; n < num_lists; n++) {
            ...
            /*
                * If a constituent within the andlist is an aux (aux==1), set aux
                * for the whole-list constituent to 2, also set aux for the smaller
                * constituent to 2, meaning they'll both be printed (as an "X").
                * (If aux is 2 for the smaller constituent going in, the same thing
                * should be done, though I doubt this ever happens.)
                */
    ...
    public void linkage_post_process(Postprocessor postprocessor) {
        ...
        for (i = 0; i < N_sublinkages; ++i) {
            ...
            if (info.improper_fat_linkage) {
                ...
                pp = sent.postProcess(postprocessor, opts, subl, false);
                /*
    ...
    /**
     * Supplies the postscript header used when generating a postscript output
     * file
     *
     * @param mode Print out the constituent tree. mode 1: treebank-style
     *             constituent tree mode 2: flat, bracketed tree [A like [B this
     *             B] A] mode 3: flat, treebank-style tree (A like (B this) )
