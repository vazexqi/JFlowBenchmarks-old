...
public class Parser {
	...
	public static void doIt(String arg[]) throws IOException {
		 ...
		 */
		while (GlobalBean.fget_input_string(input_string, opts.input, opts.out,
				opts)) {
			...
			/**
			 * Create the sentence object with the inputs
			 * "sentence string, dictionary to use, options"
			 */
			sent = new Sentence(input_string.toString(), dict, opts);
			if (sent.sentence_length() > opts
			...
			if (num_linkages == 0 && !opts.parse_options_get_batch_mode()) {
				...
				if (opts.parse_options_get_allow_null()) {
					opts.parse_options_set_min_null_count(1);
					opts.parse_options_set_max_null_count(sent
							.sentence_length());
					num_linkages = sent.sentence_parse(opts);
				}
			}
		...
		if (opts.parse_options_get_batch_mode()) {
			...
			opts.out.println("" + GlobalBean.batch_errors + " error"
	...
	public void printWordsLabelsAndLinks(Sentence sent) {
		...
		if (sent.sentence_num_linkages_found() > 0) {
			...
			num_to_query = Math.min(
					sent.sentence_num_linkages_post_processed(), 1000);
	 ...
	 * 
	 * sent = sentence_create(input_string, dict); if (sentence_parse(sent,
