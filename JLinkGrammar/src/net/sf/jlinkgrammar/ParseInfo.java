...
public class ParseInfo {
    ...
    DISNode build_DIS_CON_tree() {
        ...
        /*
           * The algorithm used here to build the DIS_CON tree depends on the
           * search percolating down from the "top" of the tree. The original
           * version of this started its search at the wall. This was fine because
           * doing a DFS from the wall explore the tree in the right order.
           *
        ...
        Arrays.sort(height_perm, 0, N_words, new Comparator() {
            public int compare(Object o1, Object o2) {
                return dfs_height[((Integer) o2).intValue()]
                        - dfs_height[((Integer) o1).intValue()];
            }
        });
    ...
    ParseSet parse_set(Sentence sent, Disjunct ld, Disjunct rd, int lw, int rw,
                       Connector le, Connector re, int cost, ParseOptions opts) {

        /*
           * returns null if there are no ways to parse, or returns a pointer to a
           * set structure representing all the ways to parse
           */
        ...
        /*
           ...
           * "parse_set() called on params that were not in the table."); }
           * Actually, we can't assert this, because of the pseudocount technique
           * that's used in count(). It's not the case that every call to
           * parse_set() has already been put into the table.
           */
            ...
            for (; m != null; m = m.next) {
                ...
                for (lcost = 0; lcost <= cost; lcost++) {
                    ...
                    Lmatch = (le != null) && (d.left != null)
                            && Connector.match(sent, le, d.left, lw, w);
                    Rmatch = (d.right != null) && (re != null)
                            && Connector.match(sent, d.right, re, w, rw);
                    for (i = 0; i < 4; i++) {
                        ls[i] = rs[i] = null;
                    }
                    if (Lmatch) {
                        ls[0] = parse_set(sent, ld, d, lw, w, le.next,
                                d.left.next, lcost, opts);
                        if (le.multi)
                            ls[1] = parse_set(sent, ld, d, lw, w, le,
                                    d.left.next, lcost, opts);
                        if (d.left.multi)
                            ls[2] = parse_set(sent, ld, d, lw, w, le.next,
                                    d.left, lcost, opts);
                        if (le.multi && d.left.multi)
                            ls[3] = parse_set(sent, ld, d, lw, w, le, d.left,
                                    lcost, opts);
                    }
                    if (Rmatch) {
                        rs[0] = parse_set(sent, d, rd, w, rw, d.right.next,
                                re.next, rcost, opts);
                        if (d.right.multi)
                            rs[1] = parse_set(sent, d, rd, w, rw, d.right,
                                    re.next, rcost, opts);
                        if (re.multi)
                            rs[2] = parse_set(sent, d, rd, w, rw, d.right.next,
                                    re, rcost, opts);
                        if (d.right.multi && re.multi)
                            rs[3] = parse_set(sent, d, rd, w, rw, d.right, re,
                                    rcost, opts);
                    }
                    ...
                    for (i = 0; i < 4; i++) {
                        ...
                        for (j = 0; j < 4; j++) {
                            if (rs[j] == null)
                                continue;
                            a_choice = new ParseChoice(ls[i], lw, w, le,
                                    d.left, rs[j], w, rw, d.right, re, ld, d,
                                    rd);
                            xt.set.put_choice_in_set(a_choice);
                        }
                    }
                    ...
                    if (ls[0] != null || ls[1] != null || ls[2] != null
                        ...
                        rset = parse_set(sent, d, rd, w, rw, d.right, re,
                                rcost, opts);
                        if (rset != null) {
                            for (i = 0; i < 4; i++) {
                                if (ls[i] == null)
                    ...
                    if ((le == null)
                            && (rs[0] != null || rs[1] != null || rs[2] != null || rs[3] != null)) {
                        /* evaluate using the right match, but not the left */
                        lset = parse_set(sent, ld, d, lw, w, le, d.left, lcost,
                                opts);
    ...
    XTableConnector x_table_pointer(int lw, int rw, Connector le, Connector re,
                                    int cost) {
    ...
    XTableConnector x_table_store(int lw, int rw, Connector le, Connector re,
                                  int cost, ParseSet set) {
    ...
    void x_table_update(int lw, int rw, Connector le, Connector re, int cost,
                        ParseSet set) {
        /*
           * Stores the value in the x_table. Unlike x_table_store, it assumes
           * it's already there
           */
        ...
        if (t == null) {
            throw new RuntimeException(
                    "This entry is supposed to be in the x_table.");
        }
        t.set = set;
    }
    ...
    int x_hash(int lw, int rw, Connector le, Connector re, int cost) {
        ...
        i = i
                + (i << 1)
                + MyRandom.randtable[(((le == null ? 0 : le.hashCode()) + i) % (x_table_size + 1))
                & (GlobalBean.RTSIZE - 1)];
        i = i
                + (i << 1)
                + MyRandom.randtable[(((re == null ? 0 : re.hashCode()) + i) % (x_table_size + 1))
                & (GlobalBean.RTSIZE - 1)];
        i = i + (i << 1)
    ...
    static CONList c_dfs(int w, DISNode start_dn, CONList c) {
        ...
        /*
           ...
           * linkages that have improper structure. Fortunately, they seem to be
           * rather rare.
           */
        CONList cx;
