...
/**
 * This is one of the core classes of the link grammar. It associates a
 * dictionary with a sentence. The First routine you have to call is.
 * Dictionary(ParseOptions opts, String dict_name, String pp_name, String
 * cons_name, String affix_name)
 * 
 * <p>
 * The dictionary format:
 * <p>
 * In what follows: Every "%" symbol and everything after it is ignored on every
 * line. Every newline or tab is replaced by a space.
 * <p>
 * The dictionary file is a sequence of ENTRIES. Each ENTRY is one or more WORDS
 * (a sequence of upper or lower case letters) separated by spaces, followed by
 * a ":", followed by an EXPRESSION followed by a ";". An EXPRESSION is a lisp
 * expression where the functions are "&" or "and" or "|" or "or", and there are
 * three types of parentheses: "()", "{}", and "[]". The terminal symbols of
 * this grammar are the connectors, which are strings of letters or numbers or
 * *s. (This description applies to the prefix form of the dictionary. the
 * current dictionary is written in infix form. If the defined constant
 * INFIX_NOTATION is defined, then infix is used otherwise prefix is used.)
 * <p>
 * The connector begins with an optinal @, which is followed by an upper case
 * sequence of letters. Each subsequent *, lower case letter or number is a
 * subscript. At the end is a + or - sign. The "@" allows this connector to
 * attach to one or more other connectors.
 * <p>
 * Here is a sample dictionary entry (in infix form):
 * <p>
 * gone: T- & {&#064EV+};
 * <p>
 * (See our paper for more about how to interpret the meaning of the dictionary
 * expressions.)
 * <p>
 * A previously defined word (such as "gone" above) may be used instead of a
 * connector to specify the expression it was defined to be. Of course, in this
 * case, it must uniquely specify a word in the dictionary, and have been
 * previously defined.
 * <p>
 * If a word is of the form "/foo", then the file current-dir/foo is a so-called
 * word file, and is read in as a list of words. A word file is just a list of
 * words separted by blanks or newlines.
 * <p>
 * A word that contains the character "_" defines an idiomatic use of the words
 * separated by the "_". For example "kind of" is an idiomatic expression, so a
 * word "kind_of" is defined in the dictionary. Idomatic expressions of any
 * number of words can be defined in this way. When the word "kind" is
 * encountered, all the idiomatic uses of the word are considered.
 * <p>
 * An expresion enclosed in "[..]" is give a cost of 1. This means that if any
 * of the connectors inside the square braces are used, a cost of 1 is incurred.
 * (This cost is the first element of the cost vector printed when a sentence is
 * parsed.) Of course if something is inside of 10 levels of "[..]" then using
 * it incurs a cost of 10. These costs are called "disjunct costs". The linkages
 * are printed out in order of non-increasing disjunct cost.
 * <p>
 * The expression "(A+ or ())" means that you can choose either "A+" or the
 * empty expression "()", that is, that the connector "A+" is optional. This is
 * more compactly expressed as "{A+}". In other words, curly braces indicate an
 * optional expression.
 * <p>
 * The expression "(A+ or [])" is the same as that above, but there is a cost of
 * 1 incurred for choosing not to use "A+". The expression "(EXP1 & [EXP2])" is
 * exactly the same as "[EXP1 & EXP2]". The difference between "({[A+]} & B+)"
 * and "([{A+}] & B+)" is that the latter always incurrs a cost of 1, while the
 * former only gets a cost of 1 if "A+" is used.
 * <p>
 * The dictionary writer is not allowed to use connectors that begin in "ID".
 * This is reserved for the connectors automatically generated for idioms.
 * <p>
 * One more thing...
 * <p>
 * <B>The Dictionary is a binary tree</b>
 * <p>
 * The data structure storing the dictionary is simply a binary tree. There is
 * one catch however. The ordering of the words is not exactly the order given
 * by strcmp. It was necessary to modify the order to make it so that "make" <
 * "make.n" < "make-up" The problem is that if some other string could lie
 * between '\0' and '.' (such as '-' which strcmp would give) then it makes it
 * much harder to return all the strings that match a given word. For example,
 * if "make-up" was inserted, then "make" was inserted the a search was done for
 * "make.n", the obvious algorithm would not find the match.
 * 
 * 
 * <CODE>
 ...
 * @see Sentence#Sentence(String input_string, Dictionary dict, ParseOptions
 *      opts)
...
public class Dictionary {
	...
	public Exp exp_list;
	/**
	 * We link together all the Exp structs that are allocated in reading this
	 ...
	 * with an extra paramater called "path". If this is non-null, then the path
	 * used to find the file is taken from that path. Otherwise the path is
	 * taken from the dict_name. This is only needed because an affix_file is
	 * opened by a recursive call to this function.
	 * 
	 * @param opts
	 *            the set of parse options is copied into the Sentence object
	 *            and Linkage Object as well as the GlobalBean Object TODO -
	 *            design the ParseOptions record keeping for better object
	 *            encapsulation
	 * @param dict_name
	 *            If this is a FQPN then it is used to set the path to all
	 *            resources
	 * @param pp_name
	 *            the make of the post processor
	 * @param cons_name
	 *            the name of the constituent file to use
	 * @param affix_name
	 *            the anme oftha affix file to use
	 * @param path
	 *            the colon separated path variable (from env?) used to find the
	 *            dictionaries
	 */
	private Dictionary(ParseOptions opts, String dict_name, String pp_name,
			String cons_name, String affix_name, String path)
			throws IOException {
		Dictionary dict;
		DictNode dict_node;
		...
		affix_table = null;
		if (affix_name != null) {
			affix_table = new Dictionary(opts, affix_name, null, null, null,
					dict_name);
		}
	...
	/**
	 * This is the dictionary constructor method. It is the equivilent of the C
	 * function create_dictionary It calls a private constructor of the same
	 * name.
	 */
	public Dictionary(ParseOptions opts, String dict_name, String pp_name,
			String cons_name, String affix_name) throws IOException {
		this(opts, dict_name, pp_name, cons_name, affix_name, null);
	}

	/**
	 * read rules from path and initialize the appropriate fields in a
	 * postprocessor structure, a pointer to which is returned. The only reason
	 * we need the dictname is to used it for the path, in case there is no
	 * DICTPATH set up. If the dictname is null, and there is no DICTPATH, it
	 * just uses the filename as the full path.
	 * <p>
	 * 
	 * @param opts
	 *            the parse options. These are kept in many places, use care!
	 * @param dictname
	 *            the dictionary to use. If fully qualified then sets the path
	 *            for affix, etc.
	 * @param path
	 *            Colon separated list of directories to search for ditionary,
	 *            postprocessor etc.
	 * 
	 * @see Postprocessor
	 * @see Dictionary#Dictionary(ParseOptions, String, String, String, String,
	 *      String)
	 */
	Postprocessor post_process_open(ParseOptions opts, String dictname,
	 ...
	 *            the fully qualified? path to the ditionary?
	...
	/**
	 * Read the dictionary into memory. This musr be preceded by opening the
	 * dictionary and seting the file pointer using open_dictionary.
	 * 
	...
	/**
	 * Returns a pointer to a lookup list of the words in the dictionary. This
	 * list is made up of DictNodes, linked by their right pointers. The node,
	 * file and string fields are copied from the dictionary.
	 * 
	 * Freeing this list elsewhere is unnecessary, as long as the rest of the
	 * program merely examines the list (doesn't change it)
	 */
	...
	/*
	 * the following routines are exactly the same as those above, only they do
	 * not consider the idiom words
	 */
	...
	/**
	 * assuming that s is a pointer to a dictionary string, and that t is a
	 * pointer to a search string, this returns 0 if they match, >0 if s>t, and
	 * <0 if s<t.
	 * <p>
	 * The matching is done as follows. Walk down the strings until you come to
	 * the end of one of them, or until you find unequal characters. A "*"
	 * matches anything. Otherwise, replace "." by "\0", and take the
	 * difference. This behavior matches that of the function dict_compare().
	 */
	...
	/**
	 * We need to prune out the lists thus generated. A sub string will only be
	 * considered a subscript if it followes the last "." in the word, and it
	...
	boolean true_dict_match(String s, String t) {
		...
		/*
		 * a dot at the end or a dot followed by a number is NOT considered a
		 * subscript
		 */
		if ((dt >= 0)
				&& (dt == t.length() - 1 || Character.isDigit(t.charAt(dt + 1))))
			dt = -1;
		if ((ds >= 0)
				&& (ds == s.length() - 1 || Character.isDigit(s.charAt(ds + 1))))
			ds = -1;
	...
	static boolean is_initials_word(String word) {
		/*
		 * This is rather esoteric and not terribly important. It returns true
		 * if the word matches the pattern /[A-Z]\.]+/
		 */
		int i;
	...
	/**
	 * returns true iff it's an appropriately formed hyphenated word. This means
	 * all letters, numbers, or hyphens, not beginning and ending with a hyphen.
	 * 
	 * @param s
	...
	static boolean is_s_word(String s) {
		...
		if (s.charAt(i) != 's')
			return false;
		i--;
		if (s.charAt(i) == 'i' || s.charAt(i) == 'u' || s.charAt(i) == 'o'
	...
	// TODO: return boolean
	/**
	 * if the string contains a single ".", and ends in ".Ix" where x is a
	 * number, return x. Return -1 if not of this form.
	 */
	...
	void dict_error(String s) throws IOException {
		...
		GlobalBean.lperrmsg = "Error parsing dictionary " + ". " + s
				+ ", line " + line_number + ", tokens = " + tokens.toString();
	}

	void warning(String s) {
		opts.out.println();
		opts.out.println("Warning: " + s);
		opts.out.println("line " + line_number + ", current token = \""
				+ token.toString() + "\"");
		opts.out.println();
	}
	...
	/**
	 * This gets the next character from the input, eliminating comments. If
	 * we're in quote mode, it does not consider the % character for comments
	 */
	...
	boolean check_connector(String s) throws IOException {
		...
		if (!Character.isUpperCase(s.charAt(j))) {
			dict_error("The first letter of a connector must be in [A-Z] (was "
					+ s.charAt(j) + ").");
			return false;
		}
		if ((s.charAt(j) == 'I') && j < s.length() - 1
				&& (s.charAt(j + 1) == 'D')) {
	...
	/**
	 * the current token is a connector (or a dictionary word) make a node for
	 * it
	 */
	...
	/**
	 * This creates a node with one child (namely e). Initializes the cost to
	 * zero
	 */
	...
	/**
	 * This creates an OR node with two children, one the given node, and the
	 * other as zeroary node. This has the effect of creating what used to be
	 * called an optional node.
	 */
	...
	/*
	 * Build (and return the root of) the tree for the expression beginning with
	 * the current token. At the end, the token is the first one not part of
	 * this expression.
	 */
	...
	/**
	 * The data structure storing the dictionary is simply a binary tree. There
	 * is one catch however. The ordering of the words is not exactly the order
	 * given by strcmp. It was necessary to modify the order to make it so that
	 * "make" < "make.n" < "make-up" The problem is that if some other string
	 * could lie between '\0' and '.' (such as '-' which strcmp would give) then
	 * it makes it much harder to return all the strings that match a given
	 * word. For example, if "make-up" was inserted, then "make" was inserted
	 * the a search was done for "make.n", the obvious algorithm would not find
	 * the match.
	 * <p>
	 * verbose version
	 * 
	...
	int dict_compare(String s, String t) {
		...
		return (i >= s.length() ? 0 : (s.charAt(i) == '.' ? 1
				: (s.charAt(i) << 1)))
				- (i >= t.length() ? 0 : (t.charAt(i) == '.' ? 1
						: (t.charAt(i) << 1)));
	}
	...
	/**
	 * Insert the new node into the dictionary below node n give error message
	 * if the new element's string is already there assumes that the "n" field
	 * of new is already set, and the left and right fields of it are null
	 */
	DictNode insert_dict(DictNode n, DictNode newNode) throws IOException {
		...
		if (comp < 0) {
			...
			dict_error("The word \"" + newNode.string
	...
	/**
	 * Insert the list into the dictionary. It does the middle one first, then
	 * the left half, then the right.
	 * 
	 * @param p
	 *            points to a list of dict_nodes connected by their left
	 *            pointers
	 * @param l
	 *            is the length of this list (the last ptr may not be null)
	 * @throws IOException
	 */
	void insert_list(DictNode p, int l) throws IOException {
		...
		if (contains_underbar(dn.string)) {
			...
			opts.out.println("*** Word \"" + dn.string + "\" found near line "
					+ line_number + ".");
			...
			opts.out.println(" found near line " + line_number
	...
	/**
	 * Starting with the current token parse one dictionary entry. Add these
	...
	boolean read_entry() throws IOException {
		...
		for (; !is_equal(':'); advance()) {
			...
			if (token.charAt(0) == '/') {
				...
				if (dn == null) {
					throw new RuntimeException("Error reading word file "
							+ token.toString() + ", file empty?");
				}
			} else {
				dn_new = new DictNode();
				dn_new.left = dn;
	...
	/**
	 * Takes as input a pointer to a DictNode. The string of this DictNode is an
	 * idiom string. This string is torn apart, and its components are inserted
	 * into the dictionary as special idiom words (ending in .I*, where * is a
	 * number). The expression of this DictNode (its node field) has already
	 * been read and constructed. This will be used to construct the special
	 * idiom expressions. The given dict node is freed. The string is also
	 * freed.
	 */
	void insert_idiom(DictNode dn) throws IOException {
		...
		if (!is_idiom_string(s)) {
			opts.out.println("*** Word \"" + s + "\" on line " + line_number
		...
		if (dn_list.right == null) {
			throw new RuntimeException(
					"Idiom string with only one connector -- should have been caught");
		}
	 ...
	 * <li>(4) links these together by their left pointers at the front of the
	 * list pointed to by dn
	...
	DictNode read_word_file(DictNode dn, String filename) throws IOException {
		...
		// Here is a bad hack for bad code. the word files are given as
		// /word/word.x.y
		// but dict open thinks that files given as "/" are abaolute. Who writes
		// code
		// like this anymore? where is localfileseperator? How do you know this
		// is UNIX?
		// ARG!!!
		// TODO - clean up file handling
		// So we now add to the bad code by saing that if the filename =
		// /word/...
		...
		if ((fp = dictopen(opts, name, file_name_copy)) == null) {
			throw new RuntimeException("Error opening word file "
					+ file_name_copy);
		}
	...
	/**
	 * This function is used to open a dictionary file or a word file, or any
	 * associated data file (like a post process knowledge file).
	 * <p>
	 * It works as follows. If the file name begins with a "/", then it's
	 * assumed to be an absolute file name and it tries to open that exact file.
	 * <p>
	 * If the filename does not begin with a "/", then it uses the dictpath
	 * mechanism to find the right file to open. This looks for the file in a
	 * sequence of directories until it finds it. The sequence of directories is
	 * specified in a dictpath string, in which each directory is followed by a
	 * ":".
	 * <p>
	 * The dictpath that it uses is constructed as follows. If the dictname is
	 * non-null, and is an absolute path name (beginning with a "/", then the
	 * part after the last "/" is removed and this is the first directory on the
	 * dictpath. After this comes the DICTPATH environment variable, followed by
	 * the DEFAULTPATH
	 */
	static Reader dictopen(ParseOptions opts, String dictname, String filename)
			throws IOException {
		...
		dp2 = "";
		/*
		 * We're no longer using the dictpath in the environment if
		 * (strlen(getenv(DICTPATH)) > 0) { sprintf(dummy2, "%s:",
		 * getenv(DICTPATH)); dp2 = dummy2; }
		 */
	...
	/**
	 * generate a new connector name obtained from the current_name allocate
	 * string space for it. return a pointer to it.
	 */
	String generate_id_connector() {
		...
		for (i = 0; i < current_name.length() - 1
				&& current_name.charAt(i) == 'A'; i++)
			;
	...
	/**
	 * Allocates string space and returns a pointer to it. In this string is
	 * placed the idiomized name of the given string s. This is the same as s,
	 * but with a postfix of ".Ix", where x is an appropriate number. x is the
	 * minimum number that distinguishes this word from others in the
	 * dictionary.
	 */
	...
	int max_postfix_found(DictNode d) {
		/*
		 * Look for words that end in ".Ix" where x is a number. Return the
		 * largest x found.
		 */
		int i, j;
		i = 0;
	...
	/**
	 * Tear the idiom string apart. Destroys the string s, but does not free it.
	 * Put the parts into a list of DictNodes (connected by their right
	 * pointers) Sets the string fields of these DictNodes pointing to the
	 * fragments of the string s. Later these will be replaced by correct names
	 * (with .Ix suffixes). The list is reversed from the way they occur in the
	 * string. A pointer to this list is returned.
	 */
