...
public class Sentence {
	 ...
	 * 
	 * @param s
	 *            sentence in String form
	 * @param opts
	 *            passes ParseOptions - In reality these are often kept in
	...
	public boolean separate_sentence(String s, ParseOptions opts) {
		...
		for (;;) {
			...
			while (k < s.length()
				...
				k++;
			if (!separate_word(s.substring(j), k - j, is_first, quote_found,
					opts))
				return false;
			is_first = false;
			j = k;
	 ...
	 */
	private boolean separate_word(String w, int wend, boolean is_first_word,
	 ...
	 * 
	 * @param opts
	 *            - not used everything comes from GlobalBean. TODO - Fix or
	 *            drop
	 */
	 ...
	 * 
	 * @param opts
	 *            parsing options used to set tolerance for nulls
	 */
	public void conjunction_prune(ParseOptions opts) {
		 ...
		 * null) && region_valid(0, sent.length, d.right, null)) {
		 * mark_region(0, sent.length, d.right, null); d.marked = true; } }
		...
		if (null_links) {
			...
			for (w = 0; w < length; w++) {
				...
				if (deletable[0][w]) {
					for (d = word[w].d; d != null; d = d.next) {
						if ((d.left == null)
								&& region_valid(w, length, d.right, null) > 0) {
	 ...
	 * 
	 * There are actually two procedures to implement this. One is mark_region()
	 * and the other is region_valid(). The latter just checks to see if the
	 ...
	 * table_lookup when nothing is known.) 0 This region can't be completed
	 ...
	 * 
	 * @param lw
	 *            integer word number of left wall
	 * @param rw
	 *            integer word number of right wall
	 * @param le
	 *            left expression
	 * @param re
	 *            right expression
	...
	public int region_valid(int lw, int rw, Connector le, Connector re) {
		...
		for (w = start_word; w <= end_word; w++) {
			...
			for (; m != null; m = m.next) {
				 ...
				 * Could eliminate by always saying "region_valid(...) != 0"
				 */
				left_valid = (((le != null) && (d.left != null) && Connector
						.prune_match(this, le, d.left, lw, w)) && ((region_valid(
						lw, w, le.next, d.left.next) > 0)
						|| ((le.multi) && region_valid(lw, w, le, d.left.next) > 0)
						|| ((d.left.multi) && region_valid(lw, w, le.next,
								d.left) > 0) || ((le.multi && d.left.multi) && region_valid(
						lw, w, le, d.left) > 0)));
				...
				right_valid = (((d.right != null) && (re != null) && Connector
						.prune_match(this, d.right, re, w, rw)) && ((region_valid(
						w, rw, d.right.next, re.next) > 0)
						|| ((d.right.multi) && region_valid(w, rw, d.right,
								re.next) > 0)
						|| ((re.multi) && region_valid(w, rw, d.right.next, re) > 0) || ((d.right.multi && re.multi) && region_valid(
						w, rw, d.right, re) > 0)));
				if ((left_valid && right_valid)
						|| (right_valid && region_valid(lw, w, le, d.left) > 0)) {
					found = 1;
					break;
				}
			}
	 ...
	 * 
	 * @param w
	 *            array index of word to match
	 * @param lc
	 *            left Connector
	 * @param lw
	 *            index into word array of left word
	 * @param rc
	 *            right Connector
	 * @param rw
	 *            index into word array of right word
	 * @return the right match
	 */
	public static MatchNode form_match_list(int w, Connector lc, int lw,
			Connector rc, int rw) {
		/* 
        */
		...
		if (lc != null) {
			ml = match_l_table[w][fast_match_hash(lc)
					& (match_l_table_size[w] - 1)];
		} else {
			ml = null;
		}
		if (rc != null) {
			mr = match_r_table[w][fast_match_hash(rc)
	...
	void table_update(int lw, int rw, Connector le, Connector re, int cost,
			int count) {
		/*
		 * Stores the value in the table. Unlike table_store, it assumes it's
		 * already there
		 */
	...
	void mark_region(int lw, int rw, Connector le, Connector re) {
		...
		for (w = start_word; w <= end_word; w++) {
			...
			for (; m != null; m = m.next) {
				 ...
				 * superfluous calls to mark_region. It didn't seem a high
	...
	Disjunct explode_disjunct_list(Disjunct d) {
		...
		for (; d != null; d = d.next) {
			d1 = Disjunct.catenate_disjuncts(d1,
					build_fat_link_substitutions(d));
		}
		return d1;
	}
		...
		for (i = 0; i < length; i++) {
			if (is_conjunction[i]) {
				word[i].d = Disjunct.catenate_disjuncts(word[i].d,
						build_AND_disjunct_list(word[i].string));
			} else {
				word[i].d = Disjunct.catenate_disjuncts(word[i].d,
						explode_disjunct_list(word[i].d));
	 ...
	 * in conjunction_prune and power_prune. Regions of length 0 are always
			...
			for (j = 0; j <= length; j++) {
				if (j == i + 1) {
					...
					deletable[i + 1][j] = false;
						...
						&& (is_conjunction[i + 1]
								|| is_conjunction[j - 1]
								|| (word[i + 1].string.equals(",") && conj_in_range(
										i + 2, j - 1)) || (word[j - 1].string
								.equals(",") && conj_in_range(j, length - 1)))) {
					deletable[i + 1][j] = true;
				} else if (j > i) {
					for (k = i + 1; k < j; k++) {
	 ...
	 * 
	 * @param lw
	 *            integer index of left word
	 * @param rw
	 *            integer index of right word
	...
	private void set_connector_length_limits(ParseOptions opts) {
		...
		for (i = 0; i < length; i++) {
			for (d = word[i].d; d != null; d = d.next) {
				set_connector_list_length_limit(d.left,
						dict.unlimited_connector_set, len, opts);
				set_connector_list_length_limit(d.right,
						dict.unlimited_connector_set, len, opts);
			}
		}
	}

	private void set_connector_list_length_limit(Connector c,
	 ...
	 * sentence_parse. If another call to sentence_parse is made on the same
	 ...
	 */
	public static int table_lookup(int lw, int rw, Connector le, Connector re,
			int cost) {
	 ...
	 */
	public static TableConnector table_pointer(int lw, int rw, Connector le,
	 ...
	 * 
	 */
	public static TableConnector table_store(int lw, int rw, Connector le,
	 ...
	 * @param dir
	 */
	public static void put_into_match_table(int size, MatchNode t[],
	 ...
	 *            the node to add
	 * @param l
	 *            the node to which we are to add m on the right
	 ...
	 * 
	 * @param opts
	 *            - refers to this.cost_cutoff that is set from ParseInfo pi at
	 ...
	 * 
	 * @param opts
	 *            unused - refers to this.cost_cutoff that is set from ParseInfo
	 *            pi at object creation. TODO - Fix where ParseInfo is kept.
	 * @param x
	 *            is the Word expression list node
	 * @param cost_cutoff
	 * @return a linked list of dijuncts for the named node
	 * @see Word#x
	 */
	public Disjunct build_disjuncts_for_XNode(ParseOptions opts, XNode x,
			int cost_cutoff) {
		Clause c;
		Disjunct dis;
		...
		for (in = 0; in < N_linkages_alloced; in++) {
			...
			if (set_has_fat_down()) {
				...
				if (only_canonical_allowed && !canonical)
					continue;
				link_info[N_linkages_post_processed] = analyze_fat_linkage(
				...
				link_info[N_linkages_post_processed] = analyze_thin_linkage(
						opts, GlobalBean.PP_SECOND_PASS);
	 ...
	 * @param sublinkage
	 */
	public void post_process_scan_linkage(Postprocessor pp, ParseOptions opts,
			Sublinkage sublinkage) {
		/*
	 ...
	 */
	public PPNode post_process(Postprocessor pp, ParseOptions opts,
	...
	public void compute_pp_link_names(Sublinkage sublinkage) {
		...
		for (i = 0; i < pi.N_links; i++) {
			...
			if (!Connector.x_match(this, sublinkage.link[i].lc,
				...
				s = Sentence.intersect_strings(sublinkage.link[i].lc.string,
	 ...
	 * analyze_fat_linkage). and then "free_digraph". For some reason this
	 * causes a space leak.
	 * 
	 * @param opts
	 * @param analyze_pass
				...
				for (in = image_array[w]; in != null; in = in.next) {
					...
					if (place == 0) {
						...
						if (upcon.label == d_label) {
							...
							dummy_connector.string = find_subdisjunct(dis,
									upcon.label).string;
						}
	 ...
	 * 
	 * @param dis
	 *            a disjunct in the label_table
	 * @param label
	 *            lable_table containing a disjunct
	...
	public void expression_prune(ParseOptions opts) {
		...
		s_table_size = MyRandom
				.next_power_of_two_up(size_of_sentence_expressions());
	...
	public void print_parse_statistics(ParseOptions opts) {
		if (sentence_num_linkages_found() > 0) {
			if (sentence_num_linkages_found() > opts
					.parse_options_get_linkage_limit()) {
				opts.out.print("Found " + sentence_num_linkages_found()
						+ " linkage"
						+ (sentence_num_linkages_found() == 1 ? "" : "s")
						+ " (" + sentence_num_valid_linkages() + " of "
						+ sentence_num_linkages_post_processed()
						+ " random linkages had no P.P. violations)");
			} else {
				opts.out.print("Found "
						+ sentence_num_linkages_post_processed() + " linkage"
						+ (sentence_num_linkages_found() == 1 ? "" : "s")
	...
	public void compute_link_names() {
		...
		ParseInfo pi = parse_info;
		for (i = 0; i < pi.N_links; i++) {
			pi.link_array[i].name = intersect_strings(
		...
		for (w = 0; w < length - 1; w++) {
			if ((word[w].string.equals(",")) && is_conjunction[w + 1]) {
				word[w].d = Disjunct.catenate_disjuncts(
						special_disjunct(GlobalBean.COMMA_LABEL, '+', "", ","),
						word[w].d);
	...
	private void construct_either() {
		...
		for (w = 0; w < length; w++) {
			...
			word[w].d = Disjunct
					.catenate_disjuncts(
							special_disjunct(GlobalBean.EITHER_LABEL, '+', "",
									"either"), word[w].d);
		}
		...
		for (w = 0; w < length; w++) {
			if (!word[w].string.equals("or"))
				continue;
			word[w].d = glom_aux_connector(word[w].d, GlobalBean.EITHER_LABEL,
					false);
		}
	}
	...
	private void construct_neither() {
		...
		for (w = 0; w < length; w++) {
			if (!word[w].string.equals("neither"))
				continue;
			word[w].d = Disjunct.catenate_disjuncts(
					special_disjunct(GlobalBean.NEITHER_LABEL, '+', "",
							"neither"), word[w].d);
		}
		...
		for (w = 0; w < length; w++) {
			if (!word[w].string.equals("nor"))
				continue;
			word[w].d = glom_aux_connector(word[w].d, GlobalBean.NEITHER_LABEL,
					true);
		}
	}
	...
	private void construct_notonlybut() {
		...
		for (w = 0; w < length; w++) {
			if (!word[w].string.equals("not"))
				continue;
			word[w].d = Disjunct.catenate_disjuncts(
					special_disjunct(GlobalBean.NOT_LABEL, '+', "", "not"),
					word[w].d);
			if (w < length - 1 && word[w + 1].string.equals("only")) {
				word[w + 1].d = Disjunct.catenate_disjuncts(
						special_disjunct(GlobalBean.NOTONLY_LABEL, '-', "",
								"only"), word[w + 1].d);
		...
		for (w = 0; w < length; w++) {
			if (!word[w].string.equals("but"))
				continue;
			word[w].d = glom_aux_connector(word[w].d, GlobalBean.NOT_LABEL,
					false);
	...
	private void construct_both() {
		...
		for (w = 0; w < length; w++) {
			if (!word[w].string.equals("both"))
				continue;
			word[w].d = Disjunct.catenate_disjuncts(
					special_disjunct(GlobalBean.BOTH_LABEL, '+', "", "both"),
					word[w].d);
		}
		...
		for (w = 0; w < length; w++) {
			if (!word[w].string.equals("and"))
				continue;
			word[w].d = glom_aux_connector(word[w].d, GlobalBean.BOTH_LABEL,
					false);
		}
	}
	 ...
	 * from the outside world is install_special_conjunctive_connectors()
	 * <p>
	 ...
	 * isn't
	 */
	public static Disjunct glom_aux_connector(Disjunct d, int label,
			boolean necessary) {
	 ...
	 */
	public static Disjunct add_one_connector(int label, int dir, String cs,
			Disjunct d) {
	 ...
	 */
	public static Disjunct special_disjunct(int label, int dir, String cs,
			String ds) {
					...
					for (dir = 0; dir < 2; dir++) {
						for (c = (dir == 1 ? d.left : d.right); c != null; c = c.next) {
							for (i = 0; i < knowledge.n_contains_one_rules; i++) {
								...
								/*
								 * printf(
								 * "pp_prune: trigger ok.  selector = %s  c.string = %s\n"
								 * , selector, c.string);
								 */
	 ...
	 */
	public int left_connector_list_update(Connector c, int word_c, int w,
			boolean shallow) {
	 ...
	 */
	public int right_connector_list_update(Connector c, int word_c, int w,
			boolean shallow) {
	...
	public void prune(ParseOptions opts) {
		...
		s_table_size = MyRandom
				.next_power_of_two_up(count_disjuncts_in_sentence());
		table = new Connector[s_table_size];
	 ...
	 */
	public boolean possible_connection(Connector lc, Connector rc,
	 ...
	 */
	public boolean right_table_search(int w, Connector c, boolean shallow,
			int word_c) {
	 ...
	 */
	public boolean left_table_search(int w, Connector c, boolean shallow,
			int word_c) {
		...
		for (w = 0; w < length; w++) {
			...
			for (d = word[w].d; d != null; d = xd) {
				...
				if ((set_dist_fields(d.left, w, -1) < 0)
						|| (set_dist_fields(d.right, w, 1) >= length)) {
					d.next = null;
				} else {
					d.next = head;
					head = d;
	 ...
	 * appropriate hash table
	 */
	public void put_into_power_table(int size, CList t[], Connector c,
			boolean shal) {
	 ...
	 * and less satisfiable. That is, rule_satisfiable(r) for a given rule r can
	...
	/*
	 * int is_in_cms_table(String str) { Cms * cms = lookup_in_cms_table(str);
