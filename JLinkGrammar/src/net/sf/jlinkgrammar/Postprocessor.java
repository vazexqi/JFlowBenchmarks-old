...
/**
 * O.K. is this a static class? No it is used in the dictionary instance object
 * which is then copied to the Sentence instance object, and then to the various
 * Linkage instance objects. The programmmer will have to pay special attention
 * to acertain wich postprocessor is used on any given linkage.
 * 
 */
public class Postprocessor {
	 ...
	 * post-processing symbol. The second one is a connector name from a link.
	 * The upper case parts must match. We imagine that the first arg is padded
	 * with an infinite sequence of "#" and that the 2nd one is padded with "*".
	 * "#" matches anything, but "*" is just like an ordinary char for matching
	 * purposes. For efficiency sake there are several different versions of
	 * these functions
	 */
	...
	/**
	 * Walk the sublinkage and apply rules. This helps determine if a linkage is
	 * valid
	 * 
	 * @param pp
	 * @param sublinkage
	 ...
	 */
	static int internal_process(Postprocessor pp, Sublinkage sublinkage,
			String msg[]) {
		int i;
		/* quick test: try applying just the relevant global rules */
		if (!apply_relevant_rules(pp, new ApplyContainsOneGlobally(),
				sublinkage, pp.knowledge.contains_one_rules,
		...
		if (!apply_relevant_rules(pp, new ApplyContainsOne(), sublinkage,
				pp.knowledge.contains_one_rules,
				pp.relevant_contains_one_rules, msg))
			return 1;
		if (!apply_relevant_rules(pp, new ApplyContainsNone(), sublinkage,
				pp.knowledge.contains_none_rules,
				pp.relevant_contains_none_rules, msg))
			return 1;
		if (!apply_rules(pp, new ApplyMustFormACycle(), sublinkage,
				pp.knowledge.form_a_cycle_rules, msg))
			return 1;
		if (!apply_rules(pp, new ApplyConnected(), sublinkage,
				pp.knowledge.connected_rules, msg))
			return 1;
		if (!apply_rules(pp, new ApplyBounded(), sublinkage,
	...
	/**
	 * returns false if the string s does not match anything in the array. The
	 * array elements are post-processing symbols
	 * 
	 * @param s
	 * @param a
	...
	/**
	 * Returns true if the linkage is connected, considering words that have at
	 * least one edge....this allows conjunctive sentences not to be thrown out.
	 * 
	 * @param pp
	...
	static boolean is_connected(Postprocessor pp) {
		/*
		 * Returns true if the linkage is connected, considering words that have
		 * at least one edge....this allows conjunctive sentences not to be
		 * thrown out.
		 */
		int i;
		for (i = 0; i < pp.pp_data.length; i++)
	...
	/**
	 * fill in the pp.pp_data.word_links array with a list of words neighboring
	 * each word (actually a list of links). The dir fields are not set, since
	 * this (after fat-link-extraction) is an undirected graph.
	 * <p>
	 ...
	 * 
	 * @param pp
	 *            Postprocessor to use - why not this.pp_data.sordlinks etc?
	 * @param sublinkage
	 *            linkage to build into graph structure
	 * 
	 */
	private static void build_graph(Postprocessor pp, Sublinkage sublinkage) {
		...
		for (link = 0; link < sublinkage.num_links; link++) {
			if (sublinkage.link[link].l == -1)
				continue;
			if (PPLinkset.PPLinkset_match(pp.knowledge.ignore_these_links,
					sublinkage.link[link].name)) {
				lol = new ListOfLinks();
	...
	/**
	 * build the domains acociated with a sublinkage. TODO - Why does this pass
	 * in a PostProcessor object? How can this.pp_data ever be filled out if we
	 * don't copy it into this instance object?
	 * 
	 * @param pp
	 * @param sublinkage
	 */
	static void build_domains(Postprocessor pp, Sublinkage sublinkage) {
		...
		for (link = 0; link < sublinkage.num_links; link++) {
			...
			if (PPLinkset.PPLinkset_match(pp.knowledge.domain_starter_links, s)) {
				...
				if (PPLinkset.PPLinkset_match(
						pp.knowledge.domain_contains_links, s))
				...
				if (PPLinkset.PPLinkset_match(
						pp.knowledge.urfl_domain_starter_links, s)) {
					setup_domain_array(pp, pp.pp_data.N_domains, s, link);
					/* always add the starter link to its urfl domain */
					add_link_to_domain(pp, link);
					bad_depth_first_search(pp, sublinkage,
							sublinkage.link[link].r, sublinkage.link[link].l,
							link);
					pp.pp_data.N_domains++;
					if (!(pp.pp_data.N_domains < GlobalBean.PP_MAX_DOMAINS)) {
						throw new RuntimeException("raise PP_MAX_DOMAINS value");
					}
				} else if (PPLinkset.PPLinkset_match(
						pp.knowledge.urfl_only_domain_starter_links, s)) {
					setup_domain_array(pp, pp.pp_data.N_domains, s, link);
					/* do not add the starter link to its urfl_only domain */
					d_depth_first_search(pp, sublinkage,
							sublinkage.link[link].l, sublinkage.link[link].l,
					...
					if (!(pp.pp_data.N_domains < GlobalBean.PP_MAX_DOMAINS)) {
						...
						pp.knowledge.left_domain_starter_links, s)) {
					setup_domain_array(pp, pp.pp_data.N_domains, s, link);
					/* do not add the starter link to a left domain */
					left_depth_first_search(pp, sublinkage,
		...
		Arrays.sort(pp.pp_data.domain_array, 0, pp.pp_data.N_domains,
				new Comparator() {

					public int compare(Object d1, Object d2) {
						return ((Domain) d1).size - ((Domain) d2).size; /*
																		 * for
																		 * sorting
																		 * the
																		 * domains
		...
		for (d = 0; d < pp.pp_data.N_domains; d++) {
			...
			if (i == -1)
				throw new RuntimeException("\tpost_process: Need an entry for "
	...
	static void build_domain_forest(Postprocessor pp, Sublinkage sublinkage) {
		...
		for (d = 0; d < pp.pp_data.N_domains - 1; d++) {
			for (d1 = d + 1; d1 < pp.pp_data.N_domains; d1++) {
				if (contained_in(pp.pp_data.domain_array[d],
						pp.pp_data.domain_array[d1], sublinkage)) {
			...
			if (d1 == pp.pp_data.N_domains) {
				 ...
				 * print_domain(d); exit(1);
				 */
			}
		}
		/*
		 * the parent links of domain nodes have been established. now do the
		 * leaves
		 */
		for (d = 0; d < pp.pp_data.N_domains; d++) {
	...
	static void depth_first_search(Postprocessor pp, Sublinkage sublinkage,
		...
		for (lol = pp.pp_data.word_links[w]; lol != null; lol = lol.next) {
			if (!pp.visited[lol.word]
					&& (lol.word != root)
					&& !(lol.word < root && lol.word < w && PPLinkset
							.PPLinkset_match(pp.knowledge.restricted_links,
									sublinkage.link[lol.link].name)))
		...
		for (lol = pp.pp_data.word_links[w]; lol != null; lol = lol.next) {
			if ((!pp.visited[lol.word])
					...
					&& !(lol.word < root && lol.word < w && PPLinkset
							.PPLinkset_match(pp.knowledge.restricted_links,
									sublinkage.link[lol.link].name)))
				bad_depth_first_search(pp, sublinkage, lol.word, root,
						start_link);
		}
	}
		...
		for (lol = pp.pp_data.word_links[w]; lol != null; lol = lol.next) {
			if (!pp.visited[lol.word]
					...
					&& !(lol.word < root && lol.word < w && PPLinkset
							.PPLinkset_match(pp.knowledge.restricted_links,
									sublinkage.link[lol.link].name)))
				d_depth_first_search(pp, sublinkage, lol.word, root, right,
						start_link);
		}
	}
	 ...
	 * @param start_link
	 */
	static void left_depth_first_search(Postprocessor pp,
			Sublinkage sublinkage, int w, int right, int start_link) {
		ListOfLinks lol;
		pp.visited[w] = true;
		for (lol = pp.pp_data.word_links[w]; lol != null; lol = lol.next) {
			if (lol.word < w && lol.link != start_link) {
	 ...
	 * @param start_link
	 */
	static void setup_domain_array(Postprocessor pp, int n, String string,
			int start_link) {
	...
	/**
	 * This is a depth first search of words reachable from w, excluding any
	 * direct edge between word a and word b.
	 * 
	 * @param pp
	 * @param sublinkage
	 * @param a
	 ...
	 * @param w
	 */
	static void reachable_without_dfs(Postprocessor pp, Sublinkage sublinkage,
			int a, int b, int w) {

		ListOfLinks lol;
		pp.visited[w] = true;
		for (lol = pp.pp_data.word_links[w]; lol != null; lol = lol.next) {
			if (!pp.visited[lol.word] && !(w == a && lol.word == b)
					&& !(w == b && lol.word == a)) {
	...
	static class ApplyContainsOne implements ApplyFn {
		 ...
		 * @param sublinkage
		 * @param rule
		 * @return returns true if and only if all groups containing the
		 *         specified link contain at least one from the required list.
		 *         (as determined by exact string matching)
		 */
		public boolean applyfn(Postprocessor pp, Sublinkage sublinkage,
				PPRule rule) {
			/*  */
			DTreeLeaf dtl;
			int d, count;
			for (d = 0; d < pp.pp_data.N_domains; d++) {
				for (dtl = pp.pp_data.domain_array[d].child; dtl != null
						&& !Postprocessor.post_process_match(rule.selector,
								sublinkage.link[dtl.link].name); dtl = dtl.next)
					;
				if (dtl != null) {
	...
	static class ApplyContainsNone implements ApplyFn {
		...
		public boolean applyfn(Postprocessor pp, Sublinkage sublinkage,
				PPRule rule) {
			/*
			 * returns true if and only if: all groups containing the selector
			 * link do not contain anything from the link_array contained in the
			 * rule. Uses exact string matching.
			 */
			DTreeLeaf dtl;
			int d;
			for (d = 0; d < pp.pp_data.N_domains; d++) {
	...
	static class ApplyContainsOneGlobally implements ApplyFn {
		...
		public boolean applyfn(Postprocessor pp, Sublinkage sublinkage,
				PPRule rule) {
			/*
			 * returns true if and only if (1) the sentence doesn't contain the
			 * selector link for the rule, or (2) it does, and it also contains
			 * one or more from the rule's link set
			 */
			...
			for (i = 0; i < sublinkage.num_links; i++) {
				if (sublinkage.link[i].l == -1)
					continue;
				if (Postprocessor.post_process_match(rule.selector,
						sublinkage.link[i].name))
					break;
			}
	 ...
	 * must_form_a_cycle list, we want to make sure that that link is in a
	 * cycle. We do this simply by deleting the link, then seeing if the end
	 * points of that link are still connected.
	 */
	...
	static class ApplyMustFormACycle implements ApplyFn {
		...
		public boolean applyfn(Postprocessor pp, Sublinkage sublinkage,
			 ...
			 * whose names are in the given list of link names. Actually, what
			 * it does is this: it returns false if the connectivity of the
			 * subgraph reachable from word 0 changes as a result of deleting
			 * these links.
			 */
			...
			for (w = 0; w < pp.pp_data.length; w++) {
				for (lol = pp.pp_data.word_links[w]; lol != null; lol = lol.next) {
					...
					if (!PPLinkset.PPLinkset_match(rule.link_set,
							sublinkage.link[lol.link].name))
						continue;
					Arrays.fill(pp.visited, false);
			...
			for (lol = pp.pp_data.links_to_ignore; lol != null; lol = lol.next) {
				...
				/*
				 * (w, lol.word) are the left and right ends of the edge we're
				 * considering
				 */
				if (!PPLinkset.PPLinkset_match(rule.link_set,
						sublinkage.link[lol.link].name))
	...
	static class ApplyBounded implements ApplyFn {
		...
		public boolean applyfn(Postprocessor pp, Sublinkage sublinkage,
			...
			/*
			 * Checks to see that all domains with this name have the property
			 * that all of the words that touch a link in the domain are not to
			 * the left of the root word of the domain.
			 */
			int d, lw, d_type;
			ListOfLinks lol;
			d_type = rule.domain;
	...
	static interface ApplyFn {
		...
		boolean applyfn(Postprocessor postprocessor, Sublinkage sublinkage,
				PPRule ppRule);
	}
	 ...
	 */
	static boolean apply_rules(Postprocessor pp, ApplyFn applyfn,
			Sublinkage sublinkage, PPRule rule_array[], String msg[]) {
		int i;
	 ...
	 */
	static boolean apply_relevant_rules(Postprocessor pp, ApplyFn applyfn,
			Sublinkage sublinkage, PPRule rule_array[], int relevant_rules[],
			String msg[]) {
		int i, idx;

		/*
		 * if we didn't accumulate link names for this sentence, we need to
		 * apply all rules
		 */
