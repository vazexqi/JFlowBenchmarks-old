...
public class Connector {
	int label;
	/*
	 * The nearest word to my left (or right) that this could connect to.
	 * Computed by power pruning
	 */
	int word;
	/*
	 * If this is a length limited connector, this gives the limit of the length
	 * of the link that can be used on this connector. Since this is strictly a
	 * funcion of the connector name, efficiency is the only reason to store
	...
	boolean connectors_equal_prune(Connector c2) {
		/*
		 * The connectors must be exactly equal. A similar function is
		 * connectors_equal_AND(), but that ignores priorities, this does not.
		 */
		return label == c2.label && multi == c2.multi
				&& priority == c2.priority && string.equals(c2.string);
	}

	boolean connector_types_equal(Connector c2) {
		/*
		 * Two connectors are said to be of the same type if they have the same
		 * label, and the initial upper case letters of their strings match.
		 */
		String s, t;
		if (label != c2.label)
			return false;
	...
	boolean connectors_equal_AND(Connector c2) {
		 ...
		 * exactly the same connectors (including lower case chars). (priorities
		 * ignored).
		 */
		return label == c2.label && multi == c2.multi
	...
	int and_connector_hash(int i) {
		/*
		 * This hash function that takes a connector and a seed value i. It only
		 * looks at the leading upper case letters of the string, and the label.
		 * This ensures that if two connectors match, then they must hash to the
		 * same place.
		 */
		String s;
		s = string;
		...
		i = i + (i << 1)
				+ MyRandom.randtable[(label + i) & (GlobalBean.RTSIZE - 1)];
		int j = 0;
	...
	static boolean match(Sentence sent, Connector a, Connector b, int aw, int bw) {
		 ...
		 * compatible (both THIN_priority, or one UP_priority and one
		 * DOWN_priority). The sequence of upper case letters must match
		 * exactly. After these comes a sequence of lower case letters "*"s or
		 * "^"s. The matching algorithm is different depending on which of the
		 * two priority cases is being considered. See the comments below.
		 */
		String s, t;
		int x, y, dist;
		if (a.label != b.label)
		...
		if (aw == 0 && bw == 0) { /*
			...
			if (!(aw < bw)) {
				throw new RuntimeException(
						"match() did not receive params in the natural order.");
			}
			dist = sent.effective_dist[aw][bw];
		}
		/*
		 * printf(
		 * "M: a=%4s b=%4s  ap=%d bp=%d  aw=%d  bw=%d  a.ll=%d b.ll=%d  dist=%d\n"
		...
		if ((x == GlobalBean.THIN_priority) && (y == GlobalBean.THIN_priority)) {
			 ...
			 * they're equal. ("^" can be used in the dictionary just like any
			 * other connector.)
			 */
			...
			return true;
		} else if ((x == GlobalBean.UP_priority)
				&& (y == GlobalBean.DOWN_priority)) {
			/*
			 * As you go up (namely from x to y) the set of strings that match
			 * (in the normal THIN sense above) should get no larger. Read the
			 * comment in and.c to understand this. In other words, the y string
			 * (t) must be weaker (or at least no stronger) that the x string
			 * (s).
			 * 
			 * This code is only correct if the strings are the same length.
			 * This is currently true, but perhaps for safty this assumption
			 * should be removed.
			 */
			while (i < s.length() && i < t.length()) {
			...
			return true;
		} else if ((y == GlobalBean.UP_priority)
				&& (x == GlobalBean.DOWN_priority)) {
			while (i < s.length() && i < t.length()) {
	...
	static boolean prune_match(Sentence sent, Connector a, Connector b, int aw,
			int bw) {
		/*
		 * This is almost identical to match(). Its reason for existance is the
		 * rather subtle fact that with "and" can transform a "Ss" connector
		 * into "Sp". This means that in order for pruning to work, we must
		 * allow a "Ss" connector on word match an "Sp" connector on a word to
		 * its right. This is what this version of match allows. we assume that
		 * a is on a word to the left of b.
		 */
		String s, t;
		int x, y, dist;
		if (a.label != b.label)
		...
		if (aw == 0 && bw == 0) { /*
			...
			if (!(aw < bw)) {
				throw new RuntimeException(
						"prune_match() did not receive params in the natural order.");
			}
			dist = sent.effective_dist[aw][bw];
		}
		/*
		 * printf(
		 * "PM: a=%4s b=%4s  ap=%d bp=%d  aw=%d  bw=%d  a.ll=%d b.ll=%d  dist=%d\n"
		 * , s, t, x, y, aw, bw, a.length_limit, b.length_limit, dist);
		 */
		if (dist > a.length_limit || dist > b.length_limit) {
			return false;
		...
		if ((x == GlobalBean.THIN_priority) && (y == GlobalBean.THIN_priority)) {
			 ...
			 */
			if (i < s.length() && i < t.length() && a.string.charAt(0) == 'S'
			 ...
			 * prevent "neither John nor I likes dogs" from being killed off. We
			 * want to allow this to apply to "are neither a dog nor a cat here"
			 * and "is neither a dog nor a cat here". This uses the "SI"
			 * connector. The third line above ensures that the connector is
			 * either "S" or "SI".
			 */
			// end PLURALIZATION
			while (i < s.length() && i < t.length()) {
				if (s.charAt(i) == '*' || t.charAt(i) == '*'
					 ...
					 * from matching '^'.....Is this necessary? ......yes, I
					 * think it is.
					 */
					i++;
				} else {
			...
			while (i < s.length() && i < t.length()) {
				if (s.charAt(i) == t.charAt(i) || s.charAt(i) == '*'
						|| t.charAt(i) == '^') {
					/*
					 * that '^' should match on the DOWN_priority node is
					 * subtle, but correct
					 */
					i++;
				} else {
