...
public class PPLexTable {
	...
	public void set_label(String label) {
		...
		for (i = 0; labels[i] != null && !labels[i].equals(label); i++)
			;
		if (labels[i] != null)
			throw new RuntimeException("pp_lexer: label " + label
					+ " multiply defined!");

		/* new label. Store it */
		if (i == GlobalBean.PP_LEXER_MAX_LABELS - 1)
			throw new RuntimeException(
					"pp_lexer: too many labels. Raise PP_LEXER_MAX_LABELS");
		labels[i] = label;
	...
	public void add_string_to_label(String str) {
		...
		if (idx_of_active_label == -1)
			throw new RuntimeException("pp_lexer: invalid syntax (line "
					+ yylineno + ")");
	...
	public void add_set_of_strings_to_label(String label_of_set) {
		...
		if ((idx_of_label_of_set = get_index_of_label(label_of_set)) == -1)
			throw new RuntimeException("pp_lexer: label " + label_of_set
	...
	static void check_string(String str) {
		if (str.length() > 1 && str.indexOf(',') >= 0)
			throw new RuntimeException("pp_lexer: string " + str
					+ " contains a comma, which is a no-no.");
	}
	...
	/**
	 * Reads a knowledge object into a table for use by other methods. Called by
	 * PPKnowledge.
	 * 
	 * @param ppknowledge
	 * @param f
	...
	public void yylex(PPKnowledge ppknowledge, Reader f) throws IOException {
		...
		while (line != null) {
			...
			if (quoteMode) {
				throw new RuntimeException("post_process: open string at line "
						+ yylineno);
			}
			if (token.length() > 0) {
				add_string_to_label(token.toString());
