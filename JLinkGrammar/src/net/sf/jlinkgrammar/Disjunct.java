...
public class Disjunct {
    ...
    static Disjunct eliminate_duplicate_disjuncts(ParseOptions opts, Disjunct d) {
        /*
           ...
           * duplicates, and returns a pointer to a new list. It frees the
           * disjuncts that are eliminated.
           */
        int i, h, count;
        Disjunct dn, dx;
        count = 0;
    ...
    boolean disjunct_types_equal(Disjunct d2) {
        /*
           ...
           * ignoring the multi fields, the priority fields, and the subscripts of
           * the connectors (and the string field of the disjunct of course).
    ...
    boolean disjuncts_equal_AND(Disjunct d2) {
        /*
           * Return true if the disjuncts are equal (ignoring priority fields) and
           * the string of the disjunct.
           */
        Connector e1, e2;
    ...
    Disjunct intersect_disjuncts(Disjunct d2) {
        /*
           * Create a new disjunct that is the GCD of d1 and d2. It assumes that
           * the disjuncts are of the same type, so the GCD will not be empty.
           */
        Disjunct d;
        Connector c1, c2, c;
        ...
        c2 = d2.left;
        while (c1 != null) {
            c.string = Sentence.intersectStrings(c1.string, c2.string);
    ...
    static Disjunct copy_disjunct(Disjunct d) {
        /*
           * This builds a new copy of the disjunct pointed to by d (except for
           * the next field which is set to null). Strings, as usual, are not
           * copied.
           */
        Disjunct d1;
        if (d == null)
