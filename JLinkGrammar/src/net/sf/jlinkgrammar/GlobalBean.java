 ...
 * needed across the entire link grammar program. This should be a parent and
 * all link grammar objects should derive from it. That way we better
 * encapsulate the variables.
 */
public class GlobalBean {
	/**
	 * This is a hack that allows one to discard disjuncts containing connectors
	 * whose cost is greater than given a bound. This number plus the cost of
	 * any connectors on a disjunct must remain negative, and this number
	...
	/**
	 * If this connector is used on the wall, then suppress the display of the
	 * wall bogus name to prevent ever suppressing
	 */
	...
	/**
	 * Some size definitions. Reduce these for small machines - Left over from C
	 * not needed in Java
	 */
	/**
	 ...
	 * 
	 * It cannot be more than 254, because I use word MAX_SENTENCE+1 to indicate
	 * that nothing can connect to this connector, and this should fit in one
	 * byte (if the word field of a connector is an (unsigned char)
	 */
	...
	/**
	 * the labels >= 0 are used by fat links while -1 is used for normal
	 * connectors
	 */
	...
	/**
	 * These parameters tell power_pruning, to tell whether this is before or
	 * after generating and disjuncts. GENTLE is before RUTHLESS is after.
	 */
	...
	/**
	 * size of random table for computing the hash functions. must be a power of
	 * 2
	 */
	 ...
	 */
	public static void left_append_string(StringBuffer string, String s,
			String t) {
	...
	public static boolean special_command(StringBuffer input_string,
			Dictionary dict) {
	...
	public static void batch_process_some_linkages(int label, Sentence sent,
			ParseOptions opts) {
		Linkage linkage;
	...
	public static int there_was_an_error(int label, Sentence sent,
			ParseOptions opts) {
		...
		if (sent.sentence_num_valid_linkages() > 0) {
			...
			if ((sent.sentence_disjunct_cost(0) == 0)
	...
	public static void process_some_linkages(Sentence sent, ParseOptions opts)
			throws IOException {
		...
		if (!opts.parse_options_get_display_bad()) {
			num_to_query = Math.min(sent.sentence_num_valid_linkages(),
					DISPLAY_MAX);
		} else {
			num_to_query = Math.min(
					sent.sentence_num_linkages_post_processed(), DISPLAY_MAX);
		}

		for (i = 0, num_displayed = 0; i < num_to_query; ++i) {

			if ((sent.sentence_num_violations(i) > 0)
					&& (!opts.parse_options_get_display_bad())) {
				continue;
			}
			...
			if (opts.verbosity > 0) {
				if (sent.sentence_num_valid_linkages() == 1
						&& (!opts.parse_options_get_display_bad())) {
				...
				opts.out.println("cost vector = (UNUSED="
						+ linkage.linkage_unused_word_cost() + " DIS="
						+ linkage.linkage_disjunct_cost() + " AND="
						+ linkage.linkage_and_cost() + " LEN="
	...
	public static int fget_input_char(InputStream in, ParseOptions opts)
			throws IOException {
	...
	public static boolean fget_input_string(StringBuffer input_string,
			InputStream in, PrintStream out, ParseOptions opts)
			throws IOException {
		int c;
